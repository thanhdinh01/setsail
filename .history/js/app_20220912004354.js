...
const listItemDTravel = document.querySelectorAll(
...
listItemDate.forEach((item) =>
  item.addEventListener("click", handleChooseSelection(listItemDate, e))
);

function handleChooseSelection(list, e) {
  list.forEach((item) => (item.style.color = ""));
  e.target.style.color = "var(--primary-color)";
  const textList = e.target.textContent;
  e.target.parentNode.parentNode.previousElementSibling.textContent = textList;
  e.target.parentNode.parentNode.classList.remove("is-show");
}
